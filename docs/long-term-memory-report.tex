...
\textbf{Supermemory}: API-first managed memory, metadata/tag partitioning, graph-memory semantics, and benchmark tooling \cite{sm-intro,sm-create,sm-filter,sm-graph,sm-bench}.\newline
\textbf{claude-mem}: hook-driven local architecture with SQLite/FTS5 and optional semantic layer \cite{cm-overview,cm-hooks,cm-db,cm-search}.\newline
\textbf{Copilot Memory}: repository-scoped managed memory with policy controls and expiry semantics, with active rollout changes in 2026 \cite{gh-memory-docs,gh-changelog-jan2026,gh-changelog-mar2026}.
...
\toprule
\textbf{System} & \textbf{Setup Speed} & \textbf{Token Predictability} & \textbf{Governance Control} & \textbf{Auditability} & \textbf{Ops Burden} & \textbf{Lock-in Risk} \\
\midrule
Cortex & \cellcolor{scoreMed}Medium & \cellcolor{scoreHi}High & \cellcolor{scoreHi}High & \cellcolor{scoreHi}High & \cellcolor{scoreMed}Medium & \cellcolor{scoreHi}Low \\
Supermemory & \cellcolor{scoreHi}High & \cellcolor{scoreMed}Medium & \cellcolor{scoreMed}Medium & \cellcolor{scoreMed}Medium & \cellcolor{scoreHi}Low & \cellcolor{scoreLow}Higher \\
claude-mem & \cellcolor{scoreMed}Medium & \cellcolor{scoreMed}Medium & \cellcolor{scoreMed}Medium--High & \cellcolor{scoreMed}Medium--High & \cellcolor{scoreLow}Higher & \cellcolor{scoreMed}Medium \\
Copilot Memory & \cellcolor{scoreHi}High & \cellcolor{scoreMed}Medium & \cellcolor{scoreMed}Medium & \cellcolor{scoreMed}Medium & \cellcolor{scoreHi}Low & \cellcolor{scoreLow}Higher \\
\bottomrule
\end{tabular}
\caption{Architecture-level scorecard from documented behavior. Not a substitute for empirical benchmark results.}
\label{tab:scorecard}
\end{table*}
...
\[
\mathrm{Total\ Cost} = \mathrm{Token\ Cost} + \mathrm{Infra\ Cost} + (\mathrm{Engineer\ Hours} \times \mathrm{Loaded\ Rate})
\]
...
\section{Benchmark Protocol (Execution-Ready)}
...
\small
\begin{itemize}
  \item Main LaTeX source: \texttt{docs/long-term-memory-report.tex}
  \item Compiled PDF: \texttt{docs/long-term-memory-report.pdf}
\end{itemize}
...
\bibitem{gh-changelog-jan2026} GitHub Changelog (Jan 15, 2026). \url{https://github.blog/changelog/2026-01-15-agentic-memory-for-github-copilot-is-in-public-preview/}
\bibitem{gh-changelog-mar2026} GitHub Changelog (Mar 4, 2026). \url{https://github.blog/changelog/2026-03-04-copilot-memory-now-on-by-default-for-pro-and-pro-users-in-public-preview/}
