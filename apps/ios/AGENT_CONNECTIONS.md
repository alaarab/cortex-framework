...
**Herdr** on a computer opens its workspace browser, named server picker and native terminal. Workspaces/tabs can be created, renamed and closed; panes can be listed, created and focused. New workspaces require an explicit folder path. Closing workspaces/tabs prompts before stopping processes. Terminal output uses bounded binary WebSocket frames with resize and acknowledgement flow control; leaving/backgrounding detaches the client and never kills its panes. Output coalesces by byte count (1 MB queued, at most 64 KB per render), preserves fragmented UTF-8/ANSI sequences, and yields between renders. Only rendered bytes are acknowledged. Transient terminal connection loss retries after 1, 2 and 4 seconds; ten healthy seconds restore that budget. Host-key changes, authentication failures, HTTP rejections and output size-limit errors do not retry. Reattachment keeps the same computer/server without refocusing a tab or replaying input; the old screen remains until fresh terminal data arrives. SwiftTerm 1.20.0 is MIT licensed; notices are included in Settings. Its reviewed build plugin only generates version metadata, so command-line builds use `-skipPackagePluginValidation`.
